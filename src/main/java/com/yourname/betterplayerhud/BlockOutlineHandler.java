  ...
  //  private static final double FACE_VISIBILITY_EPSILON = 0.01;
    ...
    private void renderBoundingBox(AxisAlignedBB aabb, double offsetX, double offsetY, double offsetZ, int color, float lineWidth, boolean isEntity) {
        ...
        if (useFlowing) {
            ...
            drawBoundingBox(wr, aabb, offsetX, offsetY, offsetZ, isEntity);
            tess.draw();
        }
    ...
    private void drawFlowingBoundingBox(double minX, double minY, double minZ, double maxX, double maxY, double maxZ, boolean isEntity) {
        ...
        if (drawAllFaces) {
            ...
            for (int i = 0; i < 12; i++) {
                ...
                setColor(RGBFlowColor.getFlowColor(midPos, totalPerimeter, speed));
                Tessellator tess = Tessellator.getInstance();
                WorldRenderer wr = tess.getWorldRenderer();
                wr.begin(GL11.GL_LINES, DefaultVertexFormats.POSITION);
    ...
    private void drawBoundingBox(WorldRenderer worldRenderer, AxisAlignedBB aabb, double offsetX, double offsetY, double offsetZ, boolean isEntity) {
        ...
        if (isEntity) {
            drawAllFaces = !BetterPlayerHUD.config.drawVisibleFacesOnlyEntities; // 使用实体专用配置
        } else {
            drawAllFaces = !BetterPlayerHUD.config.drawVisibleFacesOnlyBlocks;  // 使用方块专用配置
        }
